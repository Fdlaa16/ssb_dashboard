<script lang="ts" setup>
import StructureEditable from '@/views/dashboards/structure/StructureEditable.vue';
import type { StructureData } from '@/views/dashboards/structure/types';
import { onMounted, ref } from 'vue';
import { useRoute, useRouter } from 'vue-router';

const route = useRoute();
const router = useRouter();

const structureId = route.params.id as string;
const error = ref<string | null>(null)
const loading = ref(false)
const isFlatSnackbarVisible = ref(false)
const snackbarMessage = ref('')
const snackbarColor = ref<'success' | 'error'>('success')

const structureData = ref<StructureData>({
  id: 0,
  name: '',
  user: {
    id: 0,
    email: '',
    password: '',
  },
  date_of_birth: '',
  department: '',
  avatar: null,
});


const fetchStructure = async () => {
  loading.value = true;
  try {
    const res = await $api(`structure/${structureId}/edit`);
    const data = res.data

    structureData.value = data     
  } catch (err: any) {
    error.value = err.message || 'Gagal mengambil data structure';
  } finally {
    loading.value = false;
  }
};

onMounted(async () => {
  await fetchStructure();
});

const handleSubmit = async () => {
  try {        
    const formData = new FormData();
    formData.append('_method', 'PUT'); 

    formData.append('email', structureData.value.user.email ?? '');
    formData.append('name', structureData.value.name ?? '');
    formData.append('date_of_birth', structureData.value.date_of_birth ?? '');
    formData.append('department', structureData.value.department ?? '');
    
    if (typeof structureData.value.user.id === 'number') {
      formData.append('user_id', structureData.value.user.id.toString());
    }

    if (structureData.value.avatar instanceof File)
      formData.append('avatar', structureData.value.avatar);

    const res = await $api(`structure/${structureId}`, {
      method: 'POST',
      body: formData,
    });

    snackbarMessage.value = 'Data berhasil diperbarui!';
    snackbarColor.value = 'success';
    isFlatSnackbarVisible.value = true;

    router.push({
      name: 'dashboards-structure-list',
      query: {
        success: 'Data berhasil diperbarui!',
      },
    });
  } catch (err: any) {
    const errors = err?.data?.errors

    if (err?.status === 422 && errors) {
      const messages = Object.values(errors).flat()
      snackbarMessage.value = 'Validasi gagal: ' + messages.join(', ')
    } else {
      snackbarMessage.value = 'Gagal mengirim data: ' + (err?.message || 'Unknown error')
    }

    snackbarColor.value = 'error'
    isFlatSnackbarVisible.value = true
  }
};

const onSubmit = () => {
  handleSubmit()
}

</script>

<template>
  <VRow>
    <VCol cols="12" md="12">
      <StructureEditable
        :data="structureData"  
        @update:data="structureData = $event"
        @submit="onSubmit"
      />

    </VCol>
  </VRow>

  <VSnackbar
    v-model="isFlatSnackbarVisible"
    :color="snackbarColor"
    location="bottom start"
    variant="flat"
    timeout="3000"
  >
    {{ snackbarMessage }}
  </VSnackbar>
</template>
